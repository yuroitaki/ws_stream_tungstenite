...
async fn main() {
    ...
    let server = async move {
        let s = async_tungstenite::WebSocketStream::from_raw_socket(server_con, Role::Server, None)
            .await;
        ...
        framed
            .send("A line".to_string())
            .await
            .expect("Send a line");
        framed
            .send("A second line".to_string())
            .await
            .expect("Send a line");
    ...
    let client = async move {
        let socket =
            async_tungstenite::WebSocketStream::from_raw_socket(client_con, Role::Client, None)
                .await;
