use crate::{import::*, WsErr, WsEvent};
                ...
                while !self.events.is_empty() {
                    match ready!(pharos.as_mut().poll_ready(cx)) {
                        ...
                        Ok(()) => {
                            ...
                            //
                            if let Err(_e) = pharos
                                .as_mut()
                                .start_send(self.events.pop_front().expect("pop queued event."))
                            {
                                self.state = State::Closed;
...
impl Observable<WsEvent> for Notifier {
    ...
    fn observe(&mut self, options: ObserveConfig<WsEvent>) -> Observe<'_, WsEvent, Self::Error> {
        async move { self.pharos.observe(options).await.map_err(Into::into) }.boxed()
    }
}
