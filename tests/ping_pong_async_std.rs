...
async fn ping_pong_async_std() {
    ...
    let server = async {
        let socket = TcpListener::bind("127.0.0.1:3015")
            .await
            .expect("bind to port");
        let mut connections = socket.incoming();

        let tcp_stream = connections
            .next()
            .await
            .expect("1 connection")
            .expect("tcp connect");
        let s = accept_async(tcp_stream)
            .await
        ...
        assert_eq!(
            None,
            framed.next().await.transpose().expect("receive on framed")
        );
    };
    ...
    let client = async {
        ...
        assert_eq!(
            Some(tungstenite::Message::Pong(vec![1, 2, 3])),
            socket.next().await.transpose().expect("pong")
        );
        assert_eq!(
