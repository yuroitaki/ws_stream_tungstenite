...
use {
    ...
    futures::{executor::block_on, future::join, SinkExt, StreamExt},
...
async fn server(read_text: impl Future, steps: Progress<Step>, sc: Endpoint) {
...
async fn client(
    ...
    let (mut sink, mut stream) = WebSocketStream::from_raw_socket(cs, Role::Client, Some(conf))
        .await
        .split();

    info!("wait for send_text");
    send_text.await;
    sink.send(tungstenite::Message::Text("Text from client".to_string()))
        .await
        .expect("send text");
